describe('بررسی صفحه اجاره', () => {
  ...
  it('بررسی بخش های مختلف صفحه', () => {
    cy.visit(`${Cypress.env('DEV_HOME')}rent`)
      .title()
      .should('eq', 'سپریس | اجاره آسان خودرو - انتخاب از بین اجاره دهنده‌ها')
      .get('.banner h1')
      .should('contain', 'سِپریس، اجاره آسان خودرو')
      .get('.banner h2')
      ...
      .children('.QuestionPart')
      .should('have.class', 'activeQA');
    cy.get('.second_container .add_car_section a:nth-of-type(1)')
      .should('have.attr', 'href', '/join-us')
      .get('.second_container .add_car_section a:nth-of-type(2)')

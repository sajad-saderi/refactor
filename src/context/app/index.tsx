...
export const AppStoreWrapper = ({ children }) => {
  ...
  const context = {
    ...
    setLocation: ({ value, text, en }) => {
      ...
      setStore((store) => ({
        ...store,
        location: { id: value, fa: text, en: en },
      }));
    },
    setDate: (data) => setStore((store) => ({ ...store, date: data })),
    changingLanguage: (lan) =>
      setStore((store) => ({ ...store, activeLanguage: lan })),
  };

  return <AppStore.Provider value={context}>{children}</AppStore.Provider>;
};

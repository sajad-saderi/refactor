...
const SucceedPayment = ({ extension, language }: SuccessPayment) => {
  ...
  return (
    <article
      className='responsive minHeight local_payment'
      ...
      {renter ? (
        <section className='payment_cart'>
          <div className='title  '>
            ...
            <h4>
              {extension
                ? language.PAYMENT_PAGE.successfullExtension
                : language.PAYMENT_PAGE.successfullPurchase}
            </h4>
          </div>
          <div className='bank_track_id '>
            <p>
              {language.PAYMENT_PAGE.issueTracking}{' '}
              {numberChanger(bank_id_track.toString(), activeLanguage)}
            </p>
          </div>
            ...
            <div>
              <div className='reserve_information '>
                <div className='section_title margin_bottom_24'>
                  <img
                    src={paper_check}
                    className={language.PAYMENT_PAGE.carOptions}
                  />
                ...
                <div className='car_info padding_right_24'>
                  ...
                  {!extension && (
                    <div>
                      <p>
                        {extension ? null : language.PAYMENT_PAGE.deliveryDate}:
                      </p>
                      <span>
                        {extension
                          ? null
                  ...
                  <div>
                    <p>
                      {`${
                        extension
                          ? language.COMMON.extensionDuration
                    ...
                    {extension ? (
                      <span>
                        {numberChanger(
                          extensionInfo.no_of_extended_days.toString(),
                          activeLanguage
                        )}{' '}
                        {dynamicString(
                          null,
                          language.COMMON.day,
                          rent_search_dump.no_of_days > 1 ? true : false
                        )}
                      </span>
                    ) : (
                      <span>
                        {numberChanger(
                          rent_search_dump.no_of_days.toString(),
                          activeLanguage
                        )}{' '}
                        {dynamicString(
                          null,
                  ...
                  <div>
                    <p>
                      {extension
                        ? language.PAYMENT_PAGE.newReturnDate
                        : language.PAYMENT_PAGE.returnDate}
                      :
                    </p>
                    <span>
                      {extension
                ...
                <div>
                  <h3>
                    {extension
                      ? numberChanger(
                          (
                            extensionInfo.price + extensionInfo.insurance_price
                          ).toLocaleString(),
                          activeLanguage
                        )
                      : numberChanger(
          ...
          {!extension && (
            <div className='delivery_section margin_top_24'>
              ...
              <div className='delivery_condition'>
                ...
                <p>{language.COMMON.arrangmentForDelivery}</p>
              </div>
            </div>
          )}
          <div className='return_section margin_top_24'>
            ...
            <div className='return_condition'>
              <p className='margin_bottom_8'>
                {numberChanger(
                  dynamicString(
                    [rent_search_dump.extra_hour_price_name],
                    language.PAYMENT_PAGE.text1
                  ),
                  activeLanguage
                )}
              </p>
              <p>
                {numberChanger(
                  dynamicString(
                    [
                      rent_search_dump.max_km_per_day,
                      rent_search_dump.extra_km_price_name,
                    ],
                    language.PAYMENT_PAGE.text2
                  ),
                  activeLanguage
                )}
              </p>

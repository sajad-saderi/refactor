        ...
        <div className="cart">
          <div className="rent_info">
            ...
            <p className="date_duration">
              <span>
                {/* e.g, 99 01 23 */}
                {activeLanguage === 'fa' ? numberChanger(moment(date.from[activeLanguage].name, "jYYYY/jMM/jDD").format("jD jMMMM"),activeLanguage)
                  : numberChanger(moment(date.from[activeLanguage].name, "YYYY/MM/DD").format("D MMMM"),activeLanguage)}
                <span>
                  {/* day's name of week  */}
                  {activeLanguage === 'fa' ? moment(date.from[activeLanguage].name, "jYYYY/jMM/jDD").format("dddd")
              ...
              <span>
                {
                  ...
                  activeLanguage === 'fa' ?
                  numberChanger(moment(`${extensionInfo.end_date.jalali.y}/${extensionInfo.end_date.jalali.m}/${extensionInfo.end_date.jalali.d}`
                  ...
                  : activeLanguage === 'fa' ? numberChanger(moment(date.to[activeLanguage].name, "jYYYY/jMM/jDD").format("jD jMMMM"),activeLanguage)
                  : numberChanger(moment(date.to[activeLanguage].name, "YYYY/MM/DD").format("D MMMM"),activeLanguage)}
                <span>{
                  (extensionInfo && extensionSum.status.id === "extended") ?
                  activeLanguage === 'fa' ?
                    ...
                    :
                  activeLanguage === 'fa' ? moment(date.to[activeLanguage].name, "jYYYY/jMM/jDD").format("dddd")
                  : moment(date.to[activeLanguage].name, "YYYY/MM/DD").format("dddd")}</span>
              </span>
            </p>
          </div>
...
discounted_total_price + (extensionSum ? extensionSum.price : 0) + insurance_total_price
            ...
            <>
              ...
              {status_id === "delivered" || status_id === "paid" ? (
                <a className="renter_Cell" href={`tel:${addingCountryCodeToNumber('0' + renter_info.cell)}`}>
                  ...
                  <Icon name='phone' width='16px' height='16px' color='#4ba3ce' />
                </a>
              ) : null}
            </>
          )}
    ...
    setDownloadBox(!downloadBox)
            ...
            <Icon name="download" width="20px" height="20px" color="#3fa6da" />
              </span>
            {language.REQUEST_PAGE.contractDocuments}
            <span style={{transform:"rotate(180deg)", display:"flex"}}>
              <Icon name="chevronUp" width="20px" height="20px" color="#3fa6da" rotate={180}/>
              </span>
            </span>
            {downloadBox &&<div className={['downloadContainer',downloadBox?'containerAnimation':''].join(' ')}>
             ...
             {data.contract_url && <a className="item1" href={data.contract_url} target="_blank">
                <span><Icon name="contract" width="16px" height="16px" color="#3FA6DA"/>{language.REQUEST_PAGE.downloadContact}</span>
              </a> } 
             {data.rent_search_dump.insurance_document_url && <a href={data.rent_search_dump.insurance_document_url} target="_blank">
                <span><Icon name="shield" width="16px" height="16px" color="#3FA6DA"/>{language.REQUEST_PAGE.rentInsurance}</span>
              </a> } 
             <a href='https://core.sepris.com/static/core/sepris_checklist.pdf' target="_blank">
                <span><Icon name="checklist" width="16px" height="16px" color="#3FA6DA"/>{language.REQUEST_PAGE.rentCheckList}</span>
              </a>  
            </div>
              </div>}

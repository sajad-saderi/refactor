...
const CarPage = ({
  ...
  const languageCheck = (id) => {
    if (!searchDate.from) {
      fetchData({
        id,
        from: start_date[activeLanguage].dump,
        to: end_date[activeLanguage].dump
      });
    } else {
      fetchData({
        id,
        from: searchDate.from[activeLanguage].dump,
        to: searchDate.to[activeLanguage].dump
      });
    }
  };
  ...
  const fetchData = async (data) => {
    ...
    try {
      ...
      if (res.start_date) {
        let startDate = twoWayDateConvertor(res.start_date)[activeLanguage]
          .dump;
  ...
  const DateSetter = (id) => {
    ...
    if (router.asPath.includes('start_date')) {
      const start = twoWayDateConvertor(router.query.start_date as string)[
        activeLanguage
      ].dump;
      const end = twoWayDateConvertor(router.query.end_date as string)[
        activeLanguage
      ].dump;
    ...
    if (autoGenerateDate) {
      ...
      fetchData({
        id,
        from: from[activeLanguage].dump,
        to: to[activeLanguage].dump
      });
      toastCTX.toast_option({
    ...
    if (res.extra_hour_price > 0)
      if (res.extra_hour_price_i18n) {
        setExtra_hour_price_name(
          res.extra_hour_price_i18n.name_i18n[activeLanguage]
        );
  ...
  useEffect(() => {
    ...
    if (showCalender && calenderClick) {
      if (dayRange.from?.day && dayRange.to?.day) {
        if (
          dateObject.from[activeLanguage].dump &&
          dateObject.to[activeLanguage].dump
        ) {
          setDate({
            from: dateObject.from[activeLanguage].dump,
            to: dateObject.to[activeLanguage].dump
          });
          localStorage['date'] = JSON.stringify(dateObject);
          add_date_to_url();
        }
    ...
    <>
      {media_set.length > 0 ? (
        <>
          {expired && (
            <NextSeo
              title={`${
                ...
                language.PAGE_HEADER.carPage.title
              }`}
                    ...
                    ? [
                        {
                          ...
                          alt: `${
                            activeLanguage === 'fa' ? 'تصویر خودرو' : 'Image of'
                          } ${car_Information.car.slug[activeLanguage]}`
                        }
                      ]
                    : [],
          ...
          {/* car info section */}
          <article
            className='responsive Car_page_container'
            ...
            <section className='carInfo_container'>
              <div className='first_section_carpage'>
                <h1>
                  ...
                  <p className='size_14 year_of_build'>
                    ({year.name[activeLanguage]})
                  </p>
                        ...
                        <div className='discount_part'>
                          {total_discount_percent ? (
                            <span className='percentage'>
                              {numberChanger(
                                total_discount_percent.toLocaleString(),
                                activeLanguage
                              )}
                              %
                            </span>
                        ...
                        <div
                          ...
                          <div className='input_container'>
                            ...
                            <input
                              ...
                              value={
                                fromDay
                                  ? numberChanger(fromDay, activeLanguage)
                                  : ''
                              }
                            />
                          </div>
                          <div className='input_container'>
                            ...
                            <input
                              ...
                              value={
                                toDay
                                  ? numberChanger(toDay, activeLanguage)
                                  : ''
                              }
                            />
                          </div>
              ...
              <pre className='padding_right_24'>
                {numberChanger(cancellation_policy, activeLanguage)}
              </pre>
              {/* {with_driver && (
                <>
                  <hr />
              ...
              <div className='info_container margin_right_24 margin_bottom_24'>
                <p className='alignThem'>
                  ...
                  <span className='info_value'>
                    {body_style.name[activeLanguage]}
                  </span>
                </p>
                <p className='alignThem'>
                  <span className='info_name'>
                    {language.CAR_PAGE.transmission}
                  </span>{' '}
                  <span className='info_value'>
                ...
                {cylinder ? (
                  <p className='alignThem'>
                    <span className='info_name'>
                      {language.CAR_PAGE.cylinder}
                    </span>{' '}
                    <span className='info_value'>
                      {numberChanger(
                        cylinder.name[activeLanguage],
                        activeLanguage
                      )}
                    </span>
                  </p>
                ...
                <p className='alignThem'>
                  ...
                  <span className='info_value'>
                    {numberChanger(
                      mileage_range.name[activeLanguage],
                      activeLanguage
                    )}
                  </span>
                </p>
                <p className='alignThem'>
                  <span className='info_name'>
                    {language.CAR_PAGE.capacity}
                  </span>{' '}
                  <span className='info_value'>
                    {numberChanger(capacity.toString(), activeLanguage)}{' '}
                    {language.CAR_PAGE.persons}
                  </span>
                </p>
              </div>
              {/* <hr /> */}
                    ...
                    <div
                      ...
                      <div className='input_container'>
                        ...
                        <input
                          ...
                          readOnly={true}
                          value={
                            toDay ? numberChanger(toDay, activeLanguage) : ''
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className='owner_part_avatar_container'>
                      ...
                      {owner.rate.avg_rate_as_owner ? (
                        <p>
                          ...
                          />
                          {numberChanger(
                            owner.rate.avg_rate_as_owner.toString(),
                            activeLanguage
                          )}
                        </p>
                      ) : null}
                    ...
                    <p className='since_from'>
                      {dynamicString(
                        [
                          dateSlicer(
                            activeLanguage,
                            owner.join_date.name[activeLanguage]
                          )
                        ],
                        ...
                        {dynamicString(
                          [
                            owner.owner_avg_response_time.total_seconds >= 86400
                              ...
                              : owner.owner_avg_response_time
                              ? owner.owner_avg_response_time.name_i18n[
                                  activeLanguage
                                ]
                              : ''
                          ],
              ...
              {availableCar && !is_mine ? (
                <div className='continue_to_checkout'>
                  <span className='extra_info'>
                    {language.COMMON.paymentNote}
                  </span>
                  <Button
                    value={language.COMMON.continue}
                    customClass='localClass HEAP_Car_Btn_Continue'
                    loading={loading}
                    click={GoToCheckout}
                  />
                </div>

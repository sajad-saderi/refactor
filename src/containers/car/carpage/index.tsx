...
const CarPage = ({
  ...
  const languageCheck = (id) => {
    console.log(searchDate.from[activeLanguage]);
  ...
  useEffect(() => {
    ...
    if (!car_Information) {
      ...
      setDayRange({
        from: {
          year: +searchDate.from[activeLanguage].year,
          month: +searchDate.from[activeLanguage].month,
          day: +searchDate.from[activeLanguage].day,
        },
        to: {
          year: +searchDate.to[activeLanguage].year,
          month: +searchDate.to[activeLanguage].month,
          day: +searchDate.to[activeLanguage].day,
        },
      });
      setShowCalender(true);
  ...
  const DateSetter = (id) => {
    ...
    if (autoGenerateDate) {
      ...
      toastCTX.toast_option({
        message: "تاریخ جستجو تغییر کرده است.",
        color: "#ed9026",
        time: 0,
        autoClose: false,
      });
  ...
  const add_date_to_url = () => {
    ...
    let end = null;
    if (router.query.search_id) {
      start = convert_date_to_string_with_slash(dateObject.from[activeLanguage].dump, false);
      end = convert_date_to_string_with_slash(dateObject.to[activeLanguage].dump, false);
    } else {
      start = convert_date_to_string_with_slash(dateObject.from[activeLanguage].dump, true);
      end = convert_date_to_string_with_slash(dateObject.to[activeLanguage].dump, true);
    }
  ...
  useEffect(() => {
    ...
    if (date.from[activeLanguage].dump) {
      setDayRange({
        from: {
          year: +date.from[activeLanguage].dump.year,
          month: +date.from[activeLanguage].dump.month,
          day: +date.from[activeLanguage].dump.day
        },
        to: {
          year: +date.to[activeLanguage].dump.year,
          month: +date.to[activeLanguage].dump.month,
          day: +date.to[activeLanguage].dump.day
        }
      })
      // if (dateObject.from[activeLanguage].dump) {
      //   add_date_to_url()
